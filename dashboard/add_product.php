...
<?php 	
        ...
        elseif (mysqli_num_rows($premium)>0)
        {
        	...
        	?>
        	<title><?php echo ucfirst($premium_row['1'])." - Add Product"; ?></title>
        	<?php
            echo "
	            ...
	            <div class='change_password'>
					...
					<form class='authen_form blog_upload' method='POST' enctype='multipart/form-data'>
						...
						<textarea name='product_description' rows='5' cols='6' placeholder='Product Description' required></textarea><br>
						<label>Product Gallery</label>
						<input type='file' name='product_image' required>
						<label>Product URL</label>
						<input type='text' placeholder='Product Website URL' name='product_url' required>
            ...
            if (isset($_POST['submit']))
			{
				...
				{
					$insert_query = mysqli_query($conn,"INSERT INTO product (listing_id,member_email,product_name,product_description,product_url,product_date,product_image,listing_name) VALUES ('$listing_id','$member_email','$product_name','$product_description','$product_url','$product_date','$Filename1','$listing_name') ");
					
					if ($insert_query == TRUE) 
					{
						echo "<script>alert('Your Product is Posted')</script>";
