...
<?php 	
        ...
        {
        	$listing = mysqli_query($conn,"SELECT * FROM listing WHERE listing_email = '{$free_row['2']}' AND status = '0' ");
        	$listing_row = mysqli_fetch_row($listing);

        	include('free_sidebar.php');
        	?>
        	<title><?php echo ucfirst($free_row['1'])." - Pending Listing"; ?></title>
        	<?php
            echo "
	            <div class='bread_crumb'>
	            	<label><a href='home.php' class='a_left'>Home</a>/<a href='pending_listing.php' class='a_right'>Pending Listing</a></label>
	            </div>
	            <div class='listing'>
					<table>
						<tr>
						...
						if (mysqli_num_rows($listing)>0) 
						{
							echo '
								...
								<tr>
									...
									<td><a href="listing_details.php?id='. $listing_row['0'] .'">View</a></td>
								</tr>
        ...
        {
        	$listing = mysqli_query($conn,"SELECT * FROM listing WHERE listing_email = '{$standard_row['2']}' AND status = '0' ");
        	$listing_row = mysqli_fetch_row($listing);

        	include('standard_sidebar.php');
        	?>
        	<title><?php echo ucfirst($standard_row['1'])." - Pending Listing"; ?></title>
        	<?php
            echo "
	            <div class='bread_crumb'>
	            	<label><a href='home.php' class='a_left'>Home</a>/<a href='pending_listing.php' class='a_right'>Pending Listing</a></label>
        ...
        elseif (mysqli_num_rows($premium)>0)
        {
        	...
        	$listing = mysqli_query($conn,"SELECT * FROM listing WHERE listing_email = '{$premium_row['2']}' AND status = '0' ");
        	$listing_row = mysqli_fetch_row($listing);

        	include('premium_sidebar.php');
        	?>
        	<title><?php echo ucfirst($premium_row['1'])." - Pending Listing"; ?></title>
        	<?php
            echo "
	            <div class='bread_crumb'>
	            	<label><a href='home.php' class='a_left'>Home</a>/<a href='pending_listing.php' class='a_right'>Pending Listing</a></label>
